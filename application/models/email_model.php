...
class Email_model extends CI_Model {
    ...
    function notify_email($task = '' , $param2 = '' , $param3 = '' , $param4 = '' , $param5 = '')
    {
    	...
    	{
    		$client_id		=	$this->db->get_where('project' , array('project_id' => $param2))->row()->client_id;
    		$CLIENT_NAME	=	$this->db->get_where('client' , array('client_id' => $client_id))->row()->name;
    		$PROJECT_NAME	=	$this->db->get_where('project' , array('project_id' => $param2))->row()->title;
    		$PROJECT_LINK	=	base_url().'index.php?client/project_monitor/'.$param2;
        ...
        if ($task == 'payment_completion_notification')
        {
            ...
            $INVOICE_NUMBER =   $param2;
            $SYSTEM_PAYMENT_URL=    base_url().'index.php?client/client/invoice';

            $email_msg      =   str_replace('[INVOICE_NUMBER]' , $INVOICE_NUMBER , $email_msg);
            $email_msg      =   str_replace('[SYSTEM_PAYMENT_URL]' , $SYSTEM_PAYMENT_URL , $email_msg);

            $client_id      =   $this->db->get_where('invoice' , array('invoice_number' => $INVOICE_NUMBER))->row()->client_id;
            $email_to       =   $this->db->get_where('client' , array('client_id' => $client_id))->row()->email;
        }
        ...
        if ($task == 'new_support_ticket_notify_admin')
        {
            ...
            $SYSTEM_OPENED_TICKET_URL=    base_url().'index.php?admin/support_ticket/opened';
            ...
            $email_to       =   $this->db->get_where('admin' , array('admin_id' => 1))->row()->email;
        }
        ...
        if ($task == 'support_ticket_assign_staff')
        {
            ...
            $staff_id       =   $param3;
            $STAFF_NAME     =   $this->db->get_where('staff' , array('staff_id' => $staff_id))->row()->name;
            $SYSTEM_OPENED_TICKET_URL=    base_url().'index.php?staff/support_ticket/opened';
